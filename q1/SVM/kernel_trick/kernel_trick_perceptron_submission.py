...
def kernelize(X, Y):
    ...
    # YOUR CODE HERE
    # kernel = svm.SVC(kernel='rbf',gamma=2)
    # kernel.fit(X,Y)
    # train_predictions = kernel
    """
    ===========================================================================
    """
    ...
    for i in range(X.shape[0]):
        kern_x.append([X[i][0],X[i][1],X[i][0]**2+X[i][1]**2])

    kernelX = np.array(kern_x)
    fig = plt.figure()
...
def perceptron(X, Y):
    ...
    W = model.coef_[0]                # 3D normal
    intercept = model.intercept_[0]   # Distance from origin
